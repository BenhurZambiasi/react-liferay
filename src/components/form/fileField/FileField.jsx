...
export const FileField = ({
  ...
  const handleChangeFiles = async (e) => {
    ...
    if (file) {
      ...
      if (multiple) {
        ...
        onChange({ target: { name: name, value: aux } });
        if (ref.current) ref.current.value = "";
        return;
      }
      setFiles(file);
  ...
  const handleRemoveFile = (name) => {
    if (multiple) {
      const aux = [...files];
      const auxFilter = aux.filter((el) => name !== el.name);
      setFiles(auxFilter);
  ...
  return (
    <div
      className="un-conntainer-file-field d-flex flex-column gap-2"
      data-error={!!error}
    >
      ...
      <label className="input-file" htmlFor={name}>
        ...
        {files?.name ? (
          ...
          <>
            ...
            <span className="material-symbols-outlined icon-file-field">
              attach_file
            </span>
          </>
        )}
      ...
      {files?.name && (
        <span
          className="material-symbols-outlined delete-icon-file-field"
          onClick={() => handleRemoveFile(files.name)}
        >
          delete
        </span>
      )}
      ...
      {errorMaxSizeFile && (
        <span className="error-container filled">{errorMaxSizeFile}</span>
      )}
    </div>
  );
};
